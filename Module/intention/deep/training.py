...
import shutil
from bert_utils import MODEL_CLASSES, compute_metrics, get_intent_labels, get_slot_labels
...
def Cross_entropy_fn(outputs, tagrets):
    tagrets = tagrets.float()
    return nn.CrossEntropyLoss()(outputs, tagrets)

def Binary_entropy_fn(outputs, targets):
    targets = targets.float()
    return nn.AdaptiveLogSoftmaxWithLoss()(outputs, targets)

#============================================================
class intent_trainer:
    def __init__(self, train, val, model, config: dict):
        ...
        self.model = model
        self.loss_fn = Bce_loss_fn if config["loss"].lower() == "bce" else Cross_entropy_fn
        self.config = config
    ...
    def train(self, ckpt_path, best_model_path):
