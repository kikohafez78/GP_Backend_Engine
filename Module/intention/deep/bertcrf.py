...
class JointBert():
    def __init__(self, config, intent_label_list: list, slot_label_list: list):
        self.config = config
        ...
        self.intent_classifier = IntentClassifier(self.bert.config.hidden_size, self.num_intent_labels, config["dropout"])
        self.slot_classifier = SlotClassifier(self.bert.config.hidden_size, self.num_slot_labels, config["dropout"])
    ...
    def forward(self, input_ids, attention_mask, token_type_ids, intent_label_ids, slot_label_ids):
        ...
        return outputs # (loss), logits, (hidden states), (attentions)
